...
const MyDocument = ({ data }) => {
  ...
  return (
    <Document>
      ...
      <div
        className="flex flex-col w-full pt-10 pb-10 pl-20 pr-20 space-y-4"
        id="report"
      >
        ...
        <div className="pl-10 text-sm italic text-gray-600">
          d. Yêu cầu khác
        </div>
        <div className="flex flex-col text-sm text-gray-500 ">
          <p className="pl-10">{data && data.other}</p>
        </div>
        <div className="font-medium text-md text-gray-950 ">
          ...
          <table className="items-center w-full text-sm border border-gray-400">
            <thead className="border border-gray-400 ">
              <tr className="h-14">
                ...
                <th
                  scope="col"
                  className="relative pl-2 pr-32 border border-gray-400"
                >
                  Chuẩn đầu ra của học phần
                </th>

                <th scope="col " className="text-center border border-gray-400 ">
                  SO/PI
                </th>
              </tr>
            </thead>
                ...
                data.subjectOutputStandards.map((so) => {
                  return (
                    <tr key={so.id} className="border border-gray-400 h-14">
                      ...
                      <td className="pl-2 border relative w-[500px] border-gray-400 break-all">
                        {so.content}
                      </td>
                  ...
                  data.subjectContents.map((sc, i) => (
                    <tr key={sc.id} className="border border-gray-400 h-14">
                      ...
                      <td className="relative pl-2 break-all border border-gray-400 w-96">
                       {typeof(sc.content) === "string" && parse(sc.content)}
                      </td>
                      ...
                      <td className="relative pl-2 break-all border border-gray-400 w-72">
                        {typeof(sc.bonus) === "string" && parse(sc.bonus)}
                      </td>
                    </tr>
                  ))
                ) : (
                ...
                {data ? (
                  data.evalElements.map((ee) => (
                    <tr key={ee.id} className="border border-gray-400 h-14">
                      <th
                        scope="row"
                        className="pl-2 font-medium text-left border border-gray-400"
                      >
                        {ee.name}
                      </th>
                      <td className="relative pl-2 break-all border border-gray-400 w-96">
                        {typeof(ee.method) === "string"  && parse(ee.method )}
                      </td>
                      <td className="relative w-24 pl-2 break-all border border-gray-400">
                        {ee.proportion}
                      </td>
        ...
        <div className="pt-2 pl-8 text-sm font-bold text-gray-600">
          Trưởng bộ môn
        </div>
      </div>
      {/* </View>
